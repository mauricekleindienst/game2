  ...
  if (loading) {
    return (
      <div className="container mx-auto py-8 flex justify-center items-center h-64">
        <div className={`${loadingColor} text-xl flex items-center`}>
    ...
    <div className="relative z-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {items.map((item, index) => (
          ...
          >
            <div className={`p-4 bg-gradient-to-r from-${colorTheme.primary}-500 to-${colorTheme.secondary}-600 text-white`}>
            ...
            <div className="p-4">
              ...
              {activeItemId === item.id && (
                <div className="mt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-medium ${progressMap[String(item.id)] < 100 || loopActions ? 'text-gray-600 dark:text-gray-400' : 'text-green-600 dark:text-green-400'}`}>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 overflow-hidden">
                    <motion.div 
                      className={`h-2.5 rounded-full bg-gradient-to-r from-${colorTheme.primary}-500 to-${colorTheme.accent}-500`}
                      initial={{ width: 0 }}
              ...
              <div className={`mt-4 ${activeItemId === item.id ? 'opacity-50' : 'opacity-100'}`}>
                <button
                  className={`w-full py-2 px-4 rounded-lg font-medium transition-colors
                    ${activeItemId === item.id 
                      ? `bg-gray-200 dark:bg-gray-700 text-gray-400 dark:text-gray-500 ${loopActions ? 'cursor-pointer hover:bg-gray-300 dark:hover:bg-gray-600' : 'cursor-not-allowed'}`
